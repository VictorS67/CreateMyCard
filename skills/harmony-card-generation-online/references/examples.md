...
前提：本轮已获得合法耳机数据 schema，包含名称、仓电量、充电状态和连接状态。
下表只检查候选构造，不承诺模板检索成功；真实调用仍须完成当前权限和参数校验流程。

| 用户需求 | 候选展示字段预期 | 动作预期 |
| --- | --- | --- |
| 做个卡片，显示耳机名称和耳机盒剩余电量 | `/earphoneName`、`/batteryLevel`，不得加充电状态或连接状态 | 无明确动作，不因概述有蓝牙设置而添加 |
| 显示耳机名称、耳机盒剩余电量和耳机盒充电状态 | 上述两项加 `/chargingStatusDesc`，不得为了模板可用删掉第三项 | 无明确动作 |
| 显示耳机名称和耳机盒剩余电量，点击打开蓝牙设置 | `/earphoneName`、`/batteryLevel` | 仅选择本轮概述批准的蓝牙设置动作 |
| 做个耳机状态卡片 | 允许选择 schema 内与耳机状态直接相关的必要默认字段，不强制填满 schema | 未要求操作，不为匹配模板添加动作 |
| 显示天气温度和湿度 | 只选择本轮天气 schema 中对应的两个展示字段，不追加风力、更新时间 | 无明确动作 |
...
不能省略 `candidateOutputFields` 以绕过限制，也不能将自行补充的“充电状态”写入
`userQuery`、`title` 或 `description` 后当作用户需求。模板覆盖不足按现有路线处理，不修改输入迁就模板。
