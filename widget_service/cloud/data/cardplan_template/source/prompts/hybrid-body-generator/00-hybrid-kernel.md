...
这是受限数据语法，不是 JavaScript/TypeScript；不得输出 Markdown、解释、组件 ID、Surface、Data Path、自由颜色、自由尺寸、任意函数、URL、DeepLink 或能力调用参数。普通组件只使用“标准组件投影”展示的位置参数签名，禁止 `=`、JSX 属性或额外命名参数。局部 Template 使用 `Template("templateId@version", "size", { param: value })`；唯一的 Card 外壳使用 `Template("card@1", { ...cardParams }, content)`。所有值参数必须位于第一个 child 之前。
<!-- prompt:end -->
