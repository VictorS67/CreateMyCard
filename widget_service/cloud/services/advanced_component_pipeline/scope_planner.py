        ...
        "advancedComponents": [
            {
                ...
                "variants": _effective_candidate_variants(
                    capability,
                    task_spec,
                    effective_ids,
                ),
    ...
    return [
        {
            ...
            "content": (
                ...
                "所选高级组件的 themeIds 合集中。WeatherOverview 只表达当前天气五事实：只有 "
                "ViewWeather 同时提供 city、temperature、condition、airQuality、temperatureRange，"
                "且用户未请求小时预报、日出日落、气压、能见度、AQI 数值、体感、湿度、风、"
                "紫外线、预警、降雨概率或未来预报时"
                "才能选择；候选列表已由服务端执行相同准入过滤。DateOverview 只表达 "
                "GetCalendarEvents "
                ...
                "必须一致，0% 合法，等级与充电状态必须为可信非空字符串。只请求健康度、温度、"
                "电压、电流、充电器类型、续航/预计充满时间或外设电量时不得选择；外设电量应选"
                "BluetoothDeviceOverview。省电动作只有存在语义闭环的批准事件候选时才可进入"
                "第二层；否则仍可选择 BatteryOverview，但必须生成无动作布局。"
