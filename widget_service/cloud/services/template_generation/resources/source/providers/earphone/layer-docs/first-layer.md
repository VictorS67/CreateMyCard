...
以下核心字段收敛和自动动作规则只适用于 2×2；2×4 保持上面的规则及通用字段提取要求。
...
本轮 earphoneTemplateReference 是启用模板的完整事实来源，优先于静态示例；新增模板也必须纳入 Full 优先检查。
...
- `BluetoothDeviceOverviewMusicFull@1`：必需名称、连接状态、仓电量、更新时间；不覆盖仓充电状态或左右耳电量。
- `BluetoothDeviceOverviewCaseConnectionHero@1`：必需连接状态、仓电量。
- `BluetoothDeviceOverviewCaseSettingsHero@1`、`BluetoothDeviceOverviewStatusHero@1`：必需仓充电状态。
...
- 本轮启用参考中的全部 Full 均不可用且提供蓝牙设置候选时，以下满足条件的 Hero 回退是必须执行的规则，不是可选建议：
