...
@pytest.mark.parametrize("size,expected_limit", [("2x2", 2), ("2x4", 4)])
def test_earphone_prompt_keeps_target_branch_size_boundaries(
    size: str, expected_limit: int,
) -> None:
    task = _task().model_copy(update={"size": size})
    bindings = (_earphone_binding(["/earphoneName", "/batteryLevel"]),)
    ...
    assert ("earphoneTemplateReference" in payload) == (size == "2x2")
    assert ("【蓝牙耳机意图优先】" in system) == (size == "2x2")
    assert f'"maxItems": {expected_limit}' in system


def test_template_reference_includes_target_branch_new_full() -> None:
    fields = ["/earphoneName", "/batteryLevel", "/isConnected", "/updatedAt"]
    references = _earphone_template_reference(
        get_cardplan_registry(), (_earphone_binding(fields),),
    )
    full = next(
        item for item in references
        if item.get("templateId") == "BluetoothDeviceOverviewMusicFull@1"
    )
    assert full.get("missingInputFields") == []
    assert full.get("roles") == ["Full"]
