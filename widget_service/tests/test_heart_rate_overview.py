"""HeartRateOverview admission, direct lowering, and support placement tests."""
...
def test_heart_rate_first_layer_exposes_average_as_direct_constructor_only():
    task = _heart_task()
...
def test_heart_rate_average_lowers_as_literal_metric_without_state_or_chart(size: str):
...
@pytest.mark.parametrize("variant", ["current", "attention"])
def test_heart_rate_disabled_variants_are_rejected_by_deterministic_validation(
    variant: str,
):
    source = (
        f'SingleFocusLayout(HeartRateOverview({{"variant":"{variant}","role":"hero"}}));'
    )
...
def test_heart_rate_is_fixed_support_in_activity_multi_business_layout():
    ...
    wrong_order = (
        'HeroSupportLayout(HeartRateOverview({"variant":"average","role":"hero"}),'
