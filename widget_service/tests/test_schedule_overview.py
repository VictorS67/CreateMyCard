...
def _compile_schedule(
    ...
    projected = project_content_component_facts(selected, capability_ids, component_ids)
...
def test_schedule_rail_is_hollow_and_divider_stays_inside_business_body():
    ...
    assert 'Divider({"width":1,"height":"100%","layoutWeight":1,"strokeWidth":1,' in output
...
def test_schedule_icons_are_optional_and_semantically_checked_against_task_assets():
    ...
    assert 'Image("resources/base/media/calendar.svg", "icon", {"width":20,"height":20' in (
        compiled.effective_output
    )
